...
const asoluteSuffixPath = '.absolute-path.mdt'
...
function findMdFiles(dir) {
  ...
  files.forEach((file) => {
    ...
    } else if (file.endsWith('.mdx') && !file.endsWith(asoluteSuffixPath)) {
...
function replaceImagePaths(mdFile, originPath) {
  ...
  const newFileName = mdFile.replace(/\.mdx$/, asoluteSuffixPath)
...
function updateImagePathsInMdFiles(srcDir, originPath) {
  ...
  console.log(`${mdFiles.length} .mdx  files updated.`)
}

// 调用示例
const srcDir = './data/blog' // 替换为你的 src 目录
const originPath = 'http://localhost:3000/' // 替换为你的绝对路径前缀
